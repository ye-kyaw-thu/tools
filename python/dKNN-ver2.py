# -*- coding: utf-8 -*-
"""
Created on Sun Jul  2 13:13:10 2023
...
def draw_deepknn():
    ...
    dot.node('A1', 'ReLU Activation')
    dot.node('H2', 'Hidden Layer 2\n(Dimension: 16)')
    dot.node('A2', 'ReLU Activation')
    dot.node('K', 'K-Nearest Neighbors (KNN)')
    dot.edges([('I', 'H1'), ('H1', 'A1'), ('A1', 'H2'), ('H2', 'A2'), ('A2', 'K')])
    dot.render('deepknn', cleanup=True)

def draw_diffknn():
    dot = Digraph(format='png')
    dot.node('I', 'Input Layer\n(Dimension: d)')
    dot.node('D', 'Pairwise Distance Calculation')
    dot.node('T', 'Top-k Selection')
    dot.edges([('I', 'D'), ('D', 'T')])
    dot.render('diffknn', cleanup=True)

def main(mode, dataset, epochs, n_layers, n_neighbors, draw_diagram):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--draw_diagram", action='store_true', help="Draw and save a diagram of the neural network architecture.")
    args = parser.parse_args()

    main(args.mode, args.dataset, args.epochs, args.n_layers, args.n_neighbors, args.draw_diagram)
